...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        None => {
            ...
            let rows = stmt.query_map((), |rows| Ok((rows.get::<_, i32>(0).unwrap(), rows.get::<_, String>(1).unwrap(), rows.get::<_, PrimitiveDateTime>(2).unwrap(), rows.get::<_, bool>(3).unwrap())))?;
